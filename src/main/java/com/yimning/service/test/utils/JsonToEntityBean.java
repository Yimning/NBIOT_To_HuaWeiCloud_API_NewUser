package com.yimning.service.test.utils;

import com.google.gson.*;
import com.yimning.service.test.entity.ArrayType;
import com.yimning.service.test.entity.Json2JavaElement;
...
public class JsonToEntityBean {

    public static void main(String[] args) {
        /// 读取json字符串
        //String json = FileUtils.readToString(new File("src" + File.separator + "main" + File.separator + "resources" + File.separator + "JsonString.json"), "UTF-8");
        String json = FileUtils.readToString(new File(
                Constant.PROJECT_FILE_PATH +"/src/main/resources/JsonString.json"), "UTF-8");

        String writeToFilePath=Constant.PROJECT_FILE_PATH+"/src/main/resources/java/com/yimning/entity/deviceMessage/Message.java";
        System.out.println(json);
         //parseJson2Java(json,writeToFilePath);
    }
     ...
     */
    public static void parseJson2Java(String jsonStr,String writeToFilePath) {
        // 解析获取整个json结构集合
        ...
        String fileString = FileUtils.readToString(new File(writeToFilePath), "UTF-8");
        //System.out.println(fileString);
        int index=fileString.lastIndexOf("}");
        //System.out.println(index);
        StringBuffer stringBuffer = new StringBuffer(fileString);
        stringBuffer.insert(index-1, javaBeanStr);//给指定下标位置前的值赋新值
        ...
        // 将生成的内容写入到文件中去
        //FileUtils.writeString2File(stringBuffer.toString(), new File(writeToFilePath));
    }
    ...
    public static String getJavaFromJson(String jsonStr) {
        ...
        System.out.println("element="+jsonBeanTree.get(0));
        // 利用获取到的json结构集合,创建对应的javabean文件内容
        String javaBeanStr = createJavaBean(jsonBeanTree);
